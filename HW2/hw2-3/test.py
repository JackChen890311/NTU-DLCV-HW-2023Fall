...
def tester(encoder, classifier, discriminator, source_test_loader, target_test_loader, training_mode, verbose=False):
    if verbose:
        print("Model test ...")
    ...
    if training_mode == 'dann':
        return 100. * source_correct.item() / len(source_test_loader.dataset), 100. * target_correct.item() / len(target_test_loader.dataset), 100. * domain_correct.item() / (len(source_test_loader.dataset) + len(target_test_loader.dataset))
    else:
        return 100. * source_correct.item() / len(source_test_loader.dataset), 100. * target_correct.item() / len(target_test_loader.dataset),
